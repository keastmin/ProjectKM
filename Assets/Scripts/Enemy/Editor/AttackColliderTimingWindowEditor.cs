...
[CustomEditor(typeof(AttackColliderTimingWindow))]
public class AttackColliderTimingWindowEditor : Editor
{
    ...
    private SerializedProperty _attackColliderIdProperty;
    private SerializedProperty _openAttackColliderNormalizedTimeProperty;
    private SerializedProperty _closeAttackColliderNormalizedTimeProperty;
    ...
    private void OnEnable()
    {
        ...
        _attackColliderIdProperty = serializedObject.FindProperty("AttackColliderID");
        _openAttackColliderNormalizedTimeProperty = serializedObject.FindProperty("OpenAttackColliderNormalizedTime");
        _closeAttackColliderNormalizedTimeProperty = serializedObject.FindProperty("CloseAttackColliderNormalizedTime");

        _currentNormalizedTime = Mathf.Clamp01(_openAttackColliderNormalizedTimeProperty.floatValue);
    ...
    private void DrawDefaultInspectorFields()
    {
        ...
        EditorGUILayout.PropertyField(_attackColliderIdProperty, true);

        EditorGUILayout.Space(4f);
        EditorGUILayout.LabelField("Attack Collider Window", EditorStyles.boldLabel);

        EditorGUI.BeginChangeCheck();
        float openTime = _openAttackColliderNormalizedTimeProperty.floatValue;
        float closeTime = _closeAttackColliderNormalizedTimeProperty.floatValue;
        EditorGUILayout.MinMaxSlider(new GUIContent("Normalized Range"), ref openTime, ref closeTime, 0f, 1f);
        if (EditorGUI.EndChangeCheck())
        {
            _openAttackColliderNormalizedTimeProperty.floatValue = openTime;
            _closeAttackColliderNormalizedTimeProperty.floatValue = Mathf.Max(openTime, closeTime);
        }

        EditorGUILayout.BeginHorizontal();
        EditorGUILayout.PropertyField(_openAttackColliderNormalizedTimeProperty, new GUIContent("Open"));
        EditorGUILayout.PropertyField(_closeAttackColliderNormalizedTimeProperty, new GUIContent("Close"));
    ...
    private void DrawPreviewInspector()
    {
        ...
        if (clip == null)
        {
            EditorGUILayout.HelpBox("Assign a Preview Animation Clip to scrub the attack collider timing window.", MessageType.Info);
            StopPreviewSampling();
            return;
        }
    ...
    private void DrawTimeline(AnimationClip clip)
    {
        ...
        EditorGUI.DrawRect(windowRect, new Color(0.95f, 0.25f, 0.25f, 0.9f));
        ...
        GUI.Label(new Rect(totalRect.x + TimelinePadding, totalRect.y - 2f, 140f, 16f), "Timeline Scrub", EditorStyles.miniBoldLabel);
    ...
    private void DrawPreviewActions()
    {
        ...
        {
            _openAttackColliderNormalizedTimeProperty.floatValue = Mathf.Min(_currentNormalizedTime, _closeAttackColliderNormalizedTimeProperty.floatValue);
            serializedObject.ApplyModifiedProperties();
        }

        if (GUILayout.Button("Set Close To Current"))
        {
            _closeAttackColliderNormalizedTimeProperty.floatValue = Mathf.Max(_currentNormalizedTime, _openAttackColliderNormalizedTimeProperty.floatValue);
            serializedObject.ApplyModifiedProperties();
        }
        EditorGUILayout.EndHorizontal();
    }
    ...
    private void UpdateTimelineFromMouse(Vector2 mousePosition, Rect timelineRect)
    {
        ...
        switch (_dragMode)
        {
            case TimelineDragMode.OpenHandle:
                _openAttackColliderNormalizedTimeProperty.floatValue = Mathf.Min(normalized, _closeAttackColliderNormalizedTimeProperty.floatValue);
                _currentNormalizedTime = _openAttackColliderNormalizedTimeProperty.floatValue;
                serializedObject.ApplyModifiedProperties();
                break;

            case TimelineDragMode.CloseHandle:
                _closeAttackColliderNormalizedTimeProperty.floatValue = Mathf.Max(normalized, _openAttackColliderNormalizedTimeProperty.floatValue);
                _currentNormalizedTime = _closeAttackColliderNormalizedTimeProperty.floatValue;
                serializedObject.ApplyModifiedProperties();
                break;
    ...
    {
        _openAttackColliderNormalizedTimeProperty.floatValue = Mathf.Clamp01(_openAttackColliderNormalizedTimeProperty.floatValue);
        _closeAttackColliderNormalizedTimeProperty.floatValue = Mathf.Clamp01(_closeAttackColliderNormalizedTimeProperty.floatValue);

        if (_closeAttackColliderNormalizedTimeProperty.floatValue < _openAttackColliderNormalizedTimeProperty.floatValue)
        {
            _closeAttackColliderNormalizedTimeProperty.floatValue = _openAttackColliderNormalizedTimeProperty.floatValue;
        }
    }
}
